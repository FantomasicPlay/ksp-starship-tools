...
for x in range(0, OLM:modules:length) {
    ...
    // Pad B: система шумоподавления (DSS), траншея и залив верхней палубы -
    // отдельные модули. Их никто не выключал после старта, поэтому они
    // продолжали работать при выключенных двигателях.
    if OLM:getmodulebyindex(x):name = "DualEngineModule" set DSSmod to x.
    if OLM:getmodulebyindex(x):name = "ModuleEnginesFX" {
        if OLM:getmodulebyindex(x):gethiddenfield("runningEffectName") = "running_trench" set Trenchmod to x.
        if OLM:getmodulebyindex(x):gethiddenfield("runningEffectName") = "running_deluge" set TopDeckmod to x.
    }
}
...
function LiftOff {
    ...
    // Гасим ВСЮ водяную/шумоподавляющую систему Pad B после отрыва: раньше
    // выключался только эффект траншеи, а DSS и залив палубы продолжали лить.
    if PadB {
        if defined DSSmod if OLM:getmodulebyindex(DSSmod):hasevent("shutdown DSS") {
            OLM:getmodulebyindex(DSSmod):doevent("shutdown DSS").
        }
        if defined Trenchmod if OLM:getmodulebyindex(Trenchmod):hasevent("shutdown engine") {
            OLM:getmodulebyindex(Trenchmod):doevent("shutdown engine").
        }
        if defined TopDeckmod if OLM:getmodulebyindex(TopDeckmod):hasevent("shutdown engine") {
