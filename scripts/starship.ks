...
// Внешний оверлей включён -> встроенную телеметрию прячем, панель управления
// (кнопки Launch / De-orbit) оставляем.
set UseOverlayS to true.
clearguis().
clearscreen.
...
if not UseOverlayS VersionDisplay:show().
...
// Телеметрия для внешнего оверлея: раз в 0.1 с переписываем 0:/telemetry_ship.json.
// Обычный JSON строкой - writejson кладёт kOS-формат с $type, снаружи его неудобно парсить.
set OverlayTimerS to 0.
set sGprevT to 0.
set sGprevV to v(0,0,0).
set sGsm to 0.
set sPitch to 0.
set sEngSL to 0.
set sEngVac to 0.
set ShipLogTime to 0.
set ShipSECOTime to 0.
// Створ, который выставляем перед захватом корабля. Поле "arms open angle" в
// модуле башни не опускается ниже 5 - ноль там клампится. Полное смыкание
// делает не это поле, а действие "close arms".
set ShipArmGripAngle to 5.
set sEngMask to "".
set sEngLit to false.
when time:seconds > OverlayTimerS + 0.1 then {
    WriteShipTelemetry().
    set OverlayTimerS to time:seconds.
    return true.
}

function WriteShipTelemetry {
    if not homeconnection:isconnected return.

    // Оверлею мало общего числа: на посадке работают ЗАБОРТНЫЕ (sea level), а
    // рисовались вакуумные — они в кольце крайние. Считаем группы раздельно.
    set sEngAct to 0.
    set sEngSL to 0.
    set sEngVac to 0.
    if defined SLEngines for e in SLEngines { if e:hassuffix("thrust") if e:thrust > 10 set sEngSL to sEngSL + 1. }
    if defined VACEngines for e in VACEngines { if e:hassuffix("thrust") if e:thrust > 10 set sEngVac to sEngVac + 1. }
    set sEngAct to sEngSL + sEngVac.

    // Посопельная маска для оверлея: по символу на двигатель, 1 = горит.
    // Порядок ровно такой, как кольцо рисуется: сначала вакуумные, потом
    // забортные. Раньше уходили только счётчики, и оверлей зажигал "первые N",
    // а не те, что реально работают.
    set sEngMask to "".
    if defined VACEngines for e in VACEngines {
        set sEngLit to false.
        if e:hassuffix("thrust") if e:thrust > 10 set sEngLit to true.
        if sEngLit { set sEngMask to sEngMask + "1". } else { set sEngMask to sEngMask + "0". }
    }
    if defined SLEngines for e in SLEngines {
        set sEngLit to false.
        if e:hassuffix("thrust") if e:thrust > 10 set sEngLit to true.
        if sEngLit { set sEngMask to sEngMask + "1". } else { set sEngMask to sEngMask + "0". }
    }
    ...
    set sMet to 0.
    if missionTimer > 0 set sMet to time:seconds - missionTimer.
    // защита от старого Launch Time в settings.json
    if abs(sMet) > 86400 set sMet to 0.

    set sLoxP to 0.
    set sCh4P to 0.
    if defined shipLOX set sLoxP to shipLOX.
    if defined shipCH4 set sCh4P to shipCH4.

    // Угол корпуса для силуэта: 0 = носом вверх, растёт при завале вперёд.
    if vAng(facing:forevector, vxcl(up:vector, velocity:surface)) < 90
        set sPitch to 360 - vAng(facing:forevector, up:vector).
    else set sPitch to vAng(facing:forevector, up:vector).

    // Перегрузка. Акселерометра на борту может не быть, поэтому считаем сами:
    // полное ускорение по изменению орбитальной скорости минус гравитация.
    // Кадры идут 10 Гц и дрожат, поэтому результат сглаживаем.
    if sGprevT > 0 {
        local dtg is time:seconds - sGprevT.
        if dtg > 0.02 and dtg < 1 {
            local atot is (velocity:orbit - sGprevV) / dtg.
            local rv is ship:body:position.
            local gv is rv:normalized * (ship:body:mu / rv:sqrmagnitude).
            set sGsm to sGsm * 0.7 + ((atot - gv):mag / 9.80665) * 0.3.
        }
    }
    set sGprevV to velocity:orbit.
    set sGprevT to time:seconds.

    set sTxt to "{".
    set sTxt to sTxt + """t"":" + round(time:seconds, 2).
    set sTxt to sTxt + ",""met"":" + round(sMet, 1).
    set sTxt to sTxt + ",""phase"":""" + runningprogram + """".
    set sTxt to sTxt + ",""mission"":""" + MissionName + """".
    set sTxt to sTxt + ",""alt"":" + round(ship:altitude).
    set sTxt to sTxt + ",""spd"":" + round(airspeed, 1).
    set sTxt to sTxt + ",""vs"":" + round(verticalspeed, 1).
    set sTxt to sTxt + ",""ap"":" + round(apoapsis).
    set sTxt to sTxt + ",""pe"":" + round(periapsis).
    set sTxt to sTxt + ",""thr"":" + round(throttle * 100).
    set sTxt to sTxt + ",""mass"":" + round(ship:mass, 2).
    set sTxt to sTxt + ",""lfpct"":" + round(sCh4P, 1).
    set sTxt to sTxt + ",""oxpct"":" + round(sLoxP, 1).
    // момент посадки корабля для дуги миссии, -1 = ещё не сел
    set evsl to -1.
    if defined ShipLandTime and missionTimer > 0 set evsl to round(ShipLandTime - missionTimer, 1).
    // Пока не сел — ПРЕДСКАЗАНИЕ от Trajectories: корабль сам считает, когда
    // коснётся. Только когда он действительно идёт вниз и уже не на орбите —
    // на площадке hasimpact тоже true, а timetillimpact около нуля.
    set evsleta to -1.
    if addons:tr:hasimpact and missionTimer > 0 and verticalspeed < 0
       and periapsis < ship:body:atm:height {
        set evsleta to round(sMet + addons:tr:timetillimpact, 1).
        if evsleta < sMet + 5 set evsleta to -1.
    }
    set sTxt to sTxt + ",""evshipland"":" + evsl.
    set sTxt to sTxt + ",""evshiplandeta"":" + evsleta.
    // SECO корабля. В ShipFlightData.csv это поле писалось давно, а в живой JSON,
    // который читает оверлей, НЕ попадало вообще - поэтому веха SECO не могла
    // сработать ни разу, сколько её ни правь на стороне оверлея.
    set evsc to -1.
    if ShipSECOTime > 0 and missionTimer > 0 set evsc to round(ShipSECOTime - missionTimer, 1).
    set sTxt to sTxt + ",""evseco"":" + evsc.

    // === Предсказание SECO ===
    // Пока факта нет, время выключения считается, а не берётся из средней
    // цифры по прошлым полётам. Считать действительно недолго:
    //
    //   1) Сколько скорости ещё не хватает. MechJeb доводит орбиту в апоцентре,
    //      значит остаток - это классическая циркуляризация ТАМ:
    //          dv = sqrt(mu/rAp) - sqrt(mu*(2/rAp - 1/a))
    //      где rAp - радиус апоцентра, a - большая полуось. Ни цели, ни планов
    //      MechJeb для этого знать не нужно, всё берётся из своей же орбиты.
    //
    //   2) Сколько это по времени при переменной массе - формула Циолковского,
    //      решённая относительно t:
    //          t = (m / mdot) * (1 - e^(-dv/ve)),  ve = Isp*g0,  mdot = F/ve
    //      Масса в тоннах, тяга в кН - размерности сходятся без переводов.
    //
    // Isp берём средневзвешенный по тяге РАБОТАЮЩИХ двигателей, а не константу:
    // на вакуумных и уровневых Raptor он разный, и связка по ходу полёта меняется.
    set evsceta to -1.
    if ShipSECOTime = 0 and missionTimer > 0 and throttle > 0.05 and apoapsis > ship:body:atm:height {
        set sThrSum to 0.
        set sIspSum to 0.
        for e in ship:engines {
            if e:ignition and not e:flameout and e:availablethrust > 0 {
                set sThrSum to sThrSum + e:availablethrust.
                set sIspSum to sIspSum + e:availablethrust * e:isp.
            }
        }
        if sThrSum > 0 {
            set sVe to (sIspSum / sThrSum) * 9.80665.
            set sRap to apoapsis + ship:body:radius.
            set sMu to ship:body:mu.
            set sVcirc to sqrt(sMu / sRap).
            set sVap to sqrt(max(0, sMu * (2/sRap - 1/ship:orbit:semimajoraxis))).
            set sDv to sVcirc - sVap.
            if sDv > 0 and sVe > 0 {
                set sF to sThrSum * throttle.
                set sMdot to sF / sVe.
                if sMdot > 0 {
                    set evsceta to round(sMet + (ship:mass / sMdot) * (1 - constant:e^(-sDv / sVe)), 1).
                }
            }
        }
    }
    set sTxt to sTxt + ",""evsecoeta"":" + evsceta.
    set sTxt to sTxt + ",""engact"":" + sEngAct.
    set sTxt to sTxt + ",""steerr"":" + round(SteeringManager:angleerror, 2).
    set sTxt to sTxt + ",""pitch"":" + round(sPitch, 1).
    set sTxt to sTxt + ",""g"":" + round(sGsm, 2).
    set sTxt to sTxt + ",""engsl"":" + sEngSL + ",""engvac"":" + sEngVac.
    set sTxt to sTxt + ",""engmask"":""" + sEngMask + """".
    set sTxt to sTxt + "}".

    set sFile to open("0:/telemetry_ship.json").
    if sFile:istype("Boolean") set sFile to create("0:/telemetry_ship.json").
    sFile:clear().
    sFile:write(sTxt).

    LogShipFlightData(sMet).
}

// Полный лог полёта корабля в 0:/ShipFlightData.csv. Вызывается из
// WriteShipTelemetry, когда все величины уже посчитаны.
function LogShipFlightData {
    parameter met.
    local evSeco is -1.
    if ShipSECOTime > 0 and missionTimer > 0 set evSeco to round(ShipSECOTime - missionTimer, 1).
    if ShipLogTime = 0 {
        set ShipLogTime to time:seconds.
        LOG "MET,Phase,Alt,VSpd,Airspeed,Ap,Pe,Throttle,Mass_t,G,Pitch,SteerErr,EngSL,EngVac,CH4pct,LOXpct,EvSeco" to "0:/ShipFlightData.csv".
        return.
    }
    if time:seconds < ShipLogTime + 0.5 return.
    set ShipLogTime to time:seconds.
    ...
    LOG (round(met, 1)
        ...
        + "," + round(sCh4P, 1)
        + "," + round(sLoxP, 1)
        + "," + evSeco) to "0:/ShipFlightData.csv".
}
    ...
    set ScaleConfirm:onclick to {
        ...
        if ScaleSelectH:text = "" or ScaleSelectV:text = "" {
            ...
            if Resol[0] = -99 or Resol[1] = -99 {
                ...
                wait 0.2.
                reboot.
                if not UseOverlayS sTelemetry:show().
            }
        }
...
function Launch {
    if not AbortLaunchInProgress and not LaunchComplete {
        ...
        if RSS {
            ...
            set BoosterAp to 55000 + (cos(targetincl) * 1500) + 4000 * CargoMass / MaxCargoToOrbit.
            if ShipType = "Depot" or CargoMass > 64000 {
                set BoosterAp to 59000 + (cos(targetincl) * 1500).
        ...
        // Отмечаем SECO и У СЕБЯ: сообщение до бустера доходит не всегда, в
        // его логе EvSeco оставался -1 весь полёт. Своё событие пишем сами.
        set ShipSECOTime to time:seconds.
...
Function LaunchSteering {
    ...
    if altitude - LaunchElev < 120 {
        ...
        if apoapsis > BoosterAp - 10000 * Scale and Boosterconnected {
            ...
            if RSS {
                set result to lookDirUp(srfPrograde:vector + 0.18*up:vector, LaunchRollVector).
            } else {
...
function ReEntryAndLand {
    if addons:tr:hasimpact {
        ...
        set steeringManager:pitchpid:ki to 0.45.
        set steeringManager:pitchpid:kd to 0.75.
        set steeringManager:yawpid:kp to 0.75.
        ...
        if DynamicBanking {
            ...
            when alt:radar < 64000 * Scale then {
                set timesChecked to timesChecked + 1.
                ...
                if OLMFound when alt:radar < 58000 * Scale then if OLMFound {
                    ...
                    when Vessel(TargetedOLM):loaded then {
                        ...
                        if Vessel(TargetedOLM):partstitled("Starship Orbital Launch Mount"):length > 0 {
                            hudtext("Landingzone set on OLM",10,2,18,green,true).
                            if RSS {
                                set landingzone to latlng(round(body:geopositionof(Vessel(TargetedOLM):position):lat, 7), round(body:geopositionof(Vessel(TargetedOLM):position):lng, 7)).
                            }
                            else {
                                set landingzone to latlng(round(body:geopositionof(Vessel(TargetedOLM):position):lat, 5), round(body:geopositionof(Vessel(TargetedOLM):position):lng, 5)).
                            }
                        }
        ...
        if RSS {
            ...
            when airspeed < 7000 and ship:body:atm:sealevelpressure > 0.5 or airspeed < 3000 and ship:body:atm:sealevelpressure < 0.5 then {
                ...
                set YawPID to PIDLOOP(0.0018, 0.0000001, 0.001, -50, 50).
            }
        }
        else if KSRSS {
            when airspeed < ChangeOverSensitivity then {
        ...
        when altitude < body:atm:height then {
            ...
            when airspeed < 2150 or airspeed < 7100 and RSS then {
                ...
                when airspeed < 850 * Scale then {
                    if DynamicBanking set YawBank to 3 * max(0.2,vAng(TowerHeadingVector, vxcl(up:vector, velocity:surface))/90)^2.
                ...
                when airspeed < 600 then {
                    ...
                    if DynamicBanking set YawBank to 3 * max(0.5,vAng(TowerHeadingVector, vxcl(up:vector, velocity:surface))/90)^2.
                    else set YawBank to 1.
                    if RSS {
                    ...
                    when RadarAlt < 12000 then {
                        ...
                        if ship:body:atm:sealevelpressure > 0.5 {
                            when RadarAlt < 1800 then {
                                if (currentdeltav > maxDeltaV*1.03 or LFShip > 0.75*FuelVentCutOffValue) and ship:body:atm:sealevelpressure > 0.5 {
                                    sCMNTank:activate.
                                    when currentdeltav < maxDeltaV and LFShip < 0.75*FuelVentCutOffValue then {
                                        sCMNTank:shutdown.
                                    }
                                }
                            }
...
function ReEntrySteering {
    if not SteeringIsRunning {
        ...
        set stabalizeDamp to min((max((0.25/steeringOffset), 0.8))^1.2, 5).
...
//-----------------------Landing---------------------------///
        ...
        if LandButtonIsRunning and not LaunchButtonIsRunning and not cancelconfirmed {
            ...
            set FlipCancelRunning to true.
            set goVert to false.
            set closingPID to pidLoop(0.07, 0.04, 0.055,-5,5).
            ...
            set steeringManager:pitchpid:ki to 0.3.
            set steeringManager:pitchpid:kd to 0.5.
            set steeringManager:yawpid:kp to 0.85.
            set steeringManager:yawpid:ki to 0.3.
            ...
            if ship:body:atm:sealevelpressure > 0.5 and airspeed < 130 {
                ...
                if abs(LngLatErrorList[0]) > 20*Scale or (abs(LngLatErrorList[1]-20) > 30*Scale and RSS) or (abs(LngLatErrorList[1]) > 30*Scale and not RSS) {
                    set LandSomewhereElse to true.
                ...
                if SLEngines[1]:hassuffix("activate") SLEngines[1]:getmodule("ModuleEnginesFX"):SetField("thrust limiter", 0).
                if SLEngines[2]:hassuffix("activate") SLEngines[2]:getmodule("ModuleEnginesFX"):SetField("thrust limiter", 0).
                if abs(LngLatErrorList[0]) > 34*Scale or (abs(LngLatErrorList[1]-20) > 20*Scale and RSS) or (abs(LngLatErrorList[1]) > 20*Scale and not RSS) {
                    set LandSomewhereElse to true.
                    lock throttle to 1.
            ...
            if not (TargetOLM = "false") {
                when vAng(facing:forevector, up:vector) < 3 and Vessel(TargetOLM):distance < 2000 then {
                    ...
                    if PadB {
                        if vxcl(up:vector, Tank:position - Vessel(TargetOLM):partstitled("Starship Orbital Launch Mount 2")[0]:position):mag > 2.5*ShipHeight and vAng(vxcl(up:vector, Vessel(TargetOLM):partstitled("Starship Orbital Launch Mount 2")[0]:position - Tank:position), LandingForwardDirection) < 90 {
                            set LandSomewhereElse to true.
                            set cAbort to true.
                            SetRadarAltitude().
            ...
            when vang(-velocity:surface, ship:facing:forevector) < FlipAngleFactor * FlipAngle then {
                ...
                if TargetOLM and not cAbort {
                    when RadarAlt < 24 * ShipHeight then {
                        ...
                        when RadarAlt < 7 * ShipHeight and RadarAlt > 0.12 * ShipHeight then {
                            ...
                            when RadarAlt <  0.1 * ShipHeight then {
                                set curShipRot to ShipRot.
                                // Было ",12,24,false" - две ошибки сразу.
                                // 1) Створ 24 - это широко раскрытые руки, а не
                                //    захват; минимум поля равен 5.
                                // 2) "false" заставляет обработчик MechazillaArms
                                //    в tower.ks САМОМУ дёрнуть toggle arms, а
                                //    следом приходит CloseArms и дёргает второй
                                //    раз. Цикл башни выгребает сообщения без
                                //    паузы, модуль не успевает обновить события -
                                //    два переключения гасят друг друга, и руки
                                //    остаются раскрытыми.
                                // С "true" обработчик ничего не переключает (его
                                // ветка ждёт события "open arms", а руки и так
                                // открыты) - закрывает ровно одна команда CloseArms.
                                sendMessage(Vessel(TargetOLM), ("MechazillaArms," + round(curShipRot, 1) + ",12," + ShipArmGripAngle + ",true")).
            ...
            when (verticalspeed > -42 and throttle < ThrottleMin + 0.05 and ship:groundspeed < 6 and ThrottleMin * 3 * max(SLEngines[0]:availablethrust, 0.000001) / ship:mass > Planet1G and RadarAlt < 2*ShipHeight) or (verticalSpeed > -40 and throttle < 0.58) then {
                ...
                when ThrottleMin * 2 * max(SLEngines[0]:availablethrust, 0.000001) / ship:mass > Planet1G and throttle < ThrottleMin + 0.003 and ship:groundspeed < 1 * Scale and verticalspeed > -8 * Scale and RadarAlt > 5 or (verticalSpeed > 6*CatchVS*(Scale-0.6) and throttle < 0.5) then {
                    SLEngines[2]:shutdown.
            ...
            if not (TargetOLM = "False") {
                ...
                until time:seconds > t + 3 and (verticalSpeed > -0.1 or RadarAlt < 1) or (ship:status = "LANDED" or ship:status = "SPLASHED") and verticalspeed > -0.01 or RadarAlt < -1 or ShipLanded or AngleAbort {
                    ...
                    rcs on.
                    if alt:radar < 2 and verticalSpeed > -0.1 {
                        set ShipLanded to true.
                        if not (defined ShipLandTime) set ShipLandTime to time:seconds.
                    }
...
function LandingThrottle {
    ...
    return calcThr * max(0.75,min(abs(-verticalSpeed/8)^0.8,1)) * 1/max(min(1, 0.5*5/(-verticalSpeed) ), cos(vang(-velocity:surface, up:vector))). 
}
...
function LandingVector {
    ...
    if addons:tr:hasimpact {
        ...
        if time:seconds < LandingFlipStart + LandingFlipTime {
            ...
            if LandSomewhereElse {
                ...
                if ship:body:atm:sealevelpressure > 0.5 {
                    ...
                    set tgtError to TgtErrorVector:mag.
                    set TgtErrorStrength to max(2.8125*Scale,tgtError)/(2.8125*Scale) * max(1, 1/max(0.75,abs(RadarRatio-1))).

                    set collisionAvoider to min(1,vAng(TgtErrorVector,TowerHeadingVector)/80)^4.
                    //else set collisionAvoider to 0. if vAng(TgtErrorVector,TowerHeadingVector) > 90 
                    ...
                    if verticalSpeed < -40 and vxcl(TowerHeadingVector,TgtErrorVector):mag > (2.8125*Scale) set LndGuidVec to LndGuidVec + vxcl(TowerHeadingVector,TgtErrorVector):normalized * Scale/1.6 * abs(verticalSpeed)/90.

                    set LndSteerDamp to vAng(LndGuidVec,facing:forevector)/4 * (4*Scale)/max(0.3,TgtErrorVector:mag).
                    set result to result * 0.5 + facing:forevector * 0.5 + (LndGuidVec:normalized * angleAxis(_2SL,facing:starvector)) * angleAxis(_1SL,facing:topvector) + facing:forevector * LndsteerDamp/(LndGuidVec:mag*0.4).

                    ////Guidance
                    //set v1 to vecDraw(HeaderTank:position, TgtErrorVector:normalized * TgtErrorStrength, blue, "1",2,true,0.05).
                    //set v2 to vecDraw(HeaderTank:position, - GSVec * 0.2 * 0.75/max(0.4,RadarRatio), red, "2",2,true,0.05).
                    //set v3 to vecDraw(HeaderTank:position, PositionCorrection:normalized * min(3,TgtErrorStrength) * collisionAvoider, yellow, "3",2,true,0.05).
                    ...
                    //set vT2 to vecDraw(TargetPos, -TargetPos+myFuturePos, blue, "T",1,true,0.1).
                    //set vT3 to vecDraw(myFuturePos, PredictGSVec, red, "T",1,true,0.1).
                }
        ...
        print " ".
        print "clsPIDInput: " + round(tgtError,3).
        print "clsPID: " + round(TgtErrorStrength,3).
        print "AngularVel: " + round(AngularVel:mag,4).
    ...
    if TargetOLM and RadarAlt < 4*ShipHeight and not (LandSomewhereElse) and Vessel(TargetOLM):distance < 2000 and vAng(facing:forevector, up:vector) < 10 {
        // На Pad B детали SLE.SS.OLIT.MZ не существует вообще. Раньше здесь
        // безусловно брался [0] пустого списка -> "Index was out of range"
        // ровно на 4*ShipHeight (124 м для KSRSS) -> падал весь скрипт и посадка.
        if PadB and Vessel(TargetOLM):PARTSNAMED("PadB.Chopsticks"):length > 0 {
            set RollVector to vxcl(up:vector, Vessel(TargetOLM):PARTSNAMED("PadB.Chopsticks")[0]:position - Nose:position).
            return lookDirUp(result, RollVector).
        }
        else if Vessel(TargetOLM):PARTSNAMED("SLE.SS.OLIT.MZ"):length > 0 {
            ...
            if TargetOLM {
                if ShipSubType:contains("Block2") or ShipType:contains("Block2") or ShipType:contains("Block3") {
                    when time:seconds > ShutdownProcedureStart + 10 then {
                        sendMessage(Vessel(TargetOLM), ("MechazillaPushers,0,0.4," + round(0.3 * Scale,2) + ",false")).
                        sendMessage(Vessel(TargetOLM), ("MechazillaStabilizers," + maxstabengage)).
                    }
                    when time:seconds > ShutdownProcedureStart + 15 then {
                        sendMessage(Vessel(TargetOLM), ("MechazillaPushers,0,0.28," + round(0.3 * Scale, 2) + ",false")).
                    }
                    ...
                    when time:seconds > ShutdownProcedureStart + 10 then {
                        sendMessage(Vessel(TargetOLM), ("MechazillaPushers,0,0.4," + round(1.3 * Scale,2) + ",false")).
                        sendMessage(Vessel(TargetOLM), ("MechazillaStabilizers," + maxstabengage)).
                    }
                    when time:seconds > ShutdownProcedureStart + 15 then {
                        sendMessage(Vessel(TargetOLM), ("MechazillaPushers,0,0.28," + round(1.3 * Scale, 2) + ",false")).
                    }
                    when time:seconds > ShutdownProcedureStart + 20 then {
...
function LngLatError {
    if addons:tr:hasimpact {
        ...
        if ship:body:atm:sealevelpressure > 0.5 {
            if TargetOLM {
                if STOCK {
                    ...
                    if ShipSubType:contains("Block2") or ShipType:contains("Block2") or ShipType:contains("Block3") {
                        if RadarAlt > 6500 set LngLatOffset to -38.
                        else set LngLatOffset to -26 - vxcl(up:vector, velocity:surface):mag*0.75.
                    } else if ShipType:contains("Block1"){
        ...
        set latresult to latresult - LatOffset.
...
function LandAtOLM {
    if not (LandAtOLMisrunning) {
        ...
        if STOCK {
            ...
            if ShipType:contains("SN") 
                set FlipAltitude to 630.
            else if ShipSubType:contains("Block2") or ShipType:contains("Block2") or ShipType:contains("Block3") {
                set FlipAltitude to 760.
            } else if ShipType:contains("Block1") {
                set FlipAltitude to 810.
            } else {
                set FlipAltitude to 800.
            }
        }
        list targets in shiplist.
...
function updateTelemetry {
    ...
    for eng in SLEngines {
        ...
        if eng:hassuffix("activate") if eng:thrust > 10 set ActiveSL to ActiveSL + 1.
    }
    if not SHipType:contains("SN") for eng in VACEngines {
        if eng:hassuffix("activate") set shipThrust to shipThrust + eng:thrust.
    }
